...
const (
	...
	updateUserInfoPoint = neteaseBaseURL + "/user/updateUinfo.action"

	teamJoinGet = neteaseBaseURL + "/team/joinTeams.action"
	teamCreate  = neteaseBaseURL + "/team/create.action"
	teamQuery   = neteaseBaseURL + "/team/query.action"

	friendGet    = neteaseBaseURL + "/friend/get.action"
	friendAdd    = neteaseBaseURL + "/friend/add.action"
	friendUpdate = neteaseBaseURL + "/friend/update.action"
)
 ...
 */
func (c *ImClient) UpdateUser(u *ImUser) error {
	...
	_, err := c.post(updateUserInfoPoint, param, "info")

	if err != nil {
		fmt.Println("UpdateUser err", param, "err:", err)
		return err
	}
	fmt.Println("UpdateUser", param)
	return nil
...
func (c *ImClient) CreateImUser(u *ImUser) (*TokenInfo, error) {
	...
	infoJSON, err := c.post(createImUserPoint, param, "info")
...
func (c *ImClient) RefreshToken(accid string) (*TokenInfo, error) {
	...
	infoJSON, err := c.post(refreshTokenPoint, param, "info")
...
func (c *ImClient) UpdateToken(accid string, token string, props string) error {
	...
	_, err := c.post(updateTokenPoint, param, "info")
